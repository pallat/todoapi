.PHONY: build mysql image container
...
container:
	docker run -p:8081:8081 --env-file ./test.env --link some-mariadb:db --name runningtodo todo:test

vegeta:
	echo "GET http://:8081/stress" | vegeta attack -rate=10/s -duration=1s | vegeta report
load:
	echo "GET http://:8081/stress" | vegeta attack -rate=10/s -duration=1s > results.10qps.bin
plot:
